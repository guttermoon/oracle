...
This is the design system behind **Starboard** — brand tokens, type, logos, illustration registers, and UI-kit components, used to build brand-aligned mocks, slides, and prototypes.
...
├── fonts/                   ← brand TTF files (Playfair Display, Chivo)
├── assets/
...
- **Logo wordmark:** drawn into the logo lockup files (not shipped as a separate webfont).
...
- **Two brand fonts:** Playfair Display and Chivo. **Loaded locally from `fonts/`** (variable-weight masters). No Google Fonts dependency in the brand kit. ✅ The STARBOARD wordmark is drawn into the logo lockup files, not shipped as a font.
- The **wordmark** lives in the logo lockup files. `var(--font-family-reality)` remains as a convenience token and falls back to Playfair Display for typographic stand-ins (deck title slides, badge text).
