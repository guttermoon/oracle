# Putting the oracle draw in your newsletter
...
2. **Upload the card back for the email.** In your email platform, open the file
   manager and upload `img/back.png`. Copy its hosted URL — serving the image from
   your platform is best for deliverability. (You can skip this and hotlink from the
   site; it still works.)

---

## Option A — native blocks (works on any plan)
...
Save it as a **reusable block** in your email platform so you can drop it into any issue.

---

## Option B — paste-in HTML (if your plan has the Custom HTML block)

If your plan includes a **Custom HTML** block, you can skip the manual
...
hosted `back.png` URL). This renders the whole picker as one tidy block.
...
- **Outlook note:** the rounded gold button may render as a square in older Outlook
  ...
  use your platform's native Button block (Option A), which handles that for you.
